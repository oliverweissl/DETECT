...
def plot_comparison_row(pairs: list[tuple], texts: list[str], save_as: str = None) -> None:
    """
    Plot comparison between image pairs.

    :param pairs: The pairs to plot.
    :param texts: The texts to annotater the pairs with.
    :param save_as: Where to save the figure, if none it will only display.
    """
    fig, ax = plt.subplots(
        10,
        3,
        figsize=(6, 25),
        gridspec_kw={"width_ratios": [1, 3, 3], "wspace": 0.1, "hspace": 0.1},
        constrained_layout=True,
    )
    for i, (ax_text, ax_im1, ax_im2) in enumerate(ax):
        ax_text.text(0.5, 0.5, texts[i], ha="center", va="center")

        ax_text.set_xticks([])
        ax_text.set_yticks([])
        ax_text.set_frame_on(False)

        ax_im1.imshow(pairs[i][0])
        ax_im2.imshow(pairs[i][1])
