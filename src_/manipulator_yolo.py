...
from utils import rank_gradient_info, perturbate_s_latents, convert_to_serializable, predict_yolo
from src_.backpropagation import generate_image_with_s_latents, backpropagation_gradients_s_space_yolo
...
class ManipulatorSSpace:
    ...
    def compare_perturbed(self, s_gradients, layer_name, rank_data, prediction_target, extent_factor, top_n=0):
        ...
        predicted_idx, predicted_class, boxes_xywh, top_confidence, target_confidence = predict_yolo(self.classifier, img_perturbed_tensor, self.device, target_class=self.target_logit)
    ...
    def handle_one_seed(self,
        ...
        original_top_idx, original_top_class, original_boxes_xywh, original_top_confidence, _ = predict_yolo(self.classifier, img_tensor,
                                                                                         self.device, 2)
        if original_top_class != 2:
            # the predicted class is not car
            # TODO: handle this case
            print(f"the predicted class is {class_dict[original_top_class]} not car")
            pass
        ...
        if specified_layer is None:
            for i, (layer_name, rank_data) in enumerate(ranked_gradient_info.items()):
                ...
                for top_n in range(top_channels):
                    ...
                    if confidence_drop > confidence_drop_threshold* np.abs(original_top_confidence):
                    #    continue
