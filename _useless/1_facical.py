...
from src_.utils import load_generator, load_facial_classifier, rank_gradient_info, perturbate_s_latents, convert_to_serializable
from src_.backpropagation import generate_image_with_s_latents, backpropagation_gradients_s_space
from configs import (gan_facial_ckpt_path, segmentation_facial_ckpt_path, generate_image_base_dir, sut_facial_path,
                     preprocess_celeb_classifier, celeba_attributes_dict)
...
class ManipulatorSSpace:
    ...
    """
    def __init__(self, generator, classifier, preprocess_fn, save_dir, device=None):
        """
    ...
    def compare_perturbed(self, s_gradients, layer_name, rank_data, prediction_target, extent_factor=40, top_n=0):
        ...
        adjusted_confidence = prediction_perturbed[target_logit].cpu().detach().numpy()
...
if __name__ == "__main__":
    ...
    base_dir = os.path.join(generate_image_base_dir, 'generated_images_v3')
